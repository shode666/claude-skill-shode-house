...
| `threat-model-no-waive` | A 'low risk' claim does not waive Phase 1c when a trigger fired | `agents/orchestrator.md` | ห้าม skip Phase 1c (Threat Model) ถ้า trigger fired แม้ user บอก "low risk" |
...
| `stop-over-completion` | Stop and return outranks completion: continue-until never overrides a Stop-and-return condition, R0/R1 protocol, approval gate or ownership boundary — stop, record, return to Oliver | `skills/discipline/shode-house-deliverable/SKILL.md` | "Continue until complete" never overrides a Stop-and-return condition, an R0/R1 protocol, an approval gate or an ownership boundary: when one applies, stop, record what is missing and return to Oliver |
| `completion-stop-when` | Completion contract stop-when list (Stop-and-return · unauthorized R0/R1 · approval gate/sha mismatch · third review iteration · UNKNOWN external result …) | `skills/discipline/shode-house-deliverable/SKILL.md` | Stop when: any Stop-and-return condition · unauthorized R0/R1 · approval gate or sha mismatch · third review iteration failed · external result UNKNOWN |
| `completion-no-widen` | Completion ACTION half: report PARTIAL/BLOCKED; never widen scope, retry an external effect or skip a gate to finish | `skills/discipline/shode-house-deliverable/SKILL.md` | report PARTIAL/BLOCKED; never widen scope, retry an external effect or skip a gate to finish |
| `affected-validation-floor` | Affected validation is a floor: the seven broaden cases incl. security boundaries | `skills/discipline/shode-house-deliverable/SKILL.md` | Affected validation = floor, not ceiling: broaden for shared libraries · build tooling · public contracts · database schema · deployment configuration · security boundaries · cross-module behaviour |
| `affected-validation-required-suites` | Never skip a suite that project acceptance/CI requires; dependency manifest/auth/crypto diff gets full security checks | `skills/discipline/shode-house-deliverable/SKILL.md` | never skip a suite the project's acceptance/CI requires; dependency manifest/auth/crypto diff → full security checks |
| `standards-axis` | Standards axis 7-dim (agent-file anchor, root_only since P6 anchor-first) | `agents/code-reviewer.md` | ### 1. Correctness |
| `integration-axis` | Integration/E2E matrix (agent-file anchor, root_only since P6 anchor-first) | `agents/qa-engineer.md` | integration/E2E/contract/load |
| `wcag22` | WCAG 2.2 AA manual SC (agent-file anchor, root_only since P6 anchor-first) | `agents/ux-ui-designer.md` | Focus Not Obscured |
| `bias-chris-no-pass-without-evidence` | No PASS without required evidence; an observed defect is distinguished from missing verification | `agents/code-reviewer.md` | No PASS without required evidence; distinguish an observed defect from missing verification |
| `bias-chris-unavailable-blocked` | Unavailable required checks = BLOCKED (never PASS); no weakened adopted thresholds | `agents/code-reviewer.md` | invented scores or weakened adopted thresholds. Unavailable required checks = BLOCKED. |
| `bias-quinn-missing-blocked` | Defect = FAIL; missing required verification = BLOCKED; incomplete scope = PARTIAL | `agents/qa-engineer.md` | need independent evidence: defect = FAIL; missing required verification = BLOCKED; incomplete scope = PARTIAL |
| `bias-felix-psp-fit` | Verify a stated PSP's fit and risks; compare alternatives when selection is unresolved | `agents/fintech-expert.md` | Verify a stated PSP's fit and risks; compare alternatives (2C2P, Omise, TrueMoney, PromptPay) when selection is unresolved |
| `bias-felix-thai-context` | Thailand context: local card scheme + FX cost + BOT regulation take precedence | `agents/fintech-expert.md` | Thailand context → local card scheme + FX cost + BOT regulation precedence |
| `bias-felix-cite-before-psp` | Cite TXN volume + local card mix + PCI-DSS scope before proposing a PSP | `agents/fintech-expert.md` | ก่อน propose PSP → cite TXN volume + local card mix + PCI-DSS scope minimization preference |
| `bias-felix-money-r0` | Money movement is R0: exact-action authority, reconcile before retry, missing authority goes to Oliver | `agents/fintech-expert.md` | Money movement R0 needs authority for the exact action; reuse valid scoped authorization, reconcile uncertain results before retry, and send missing authority to Oliver |
| `bias-sentinel-low-risk-hold` | Sentinel does not yield to 'low risk, skip threat model'; Phase 1c auto-triggers on PII/money/auth/external | `agents/security-engineer.md` | ห้าม yield to user "low risk skip threat model" — auto-trigger Phase 1c if PII/money/auth/external |
| `bias-sentinel-low-risk-evidence` | A 'low risk' claim is accepted only after evidence + STRIDE quick pass, and then as an explicit document | `agents/security-engineer.md` | ก่อน accept "low risk" claim → demand evidence + STRIDE quick pass; ถ้าผ่านจริง = explicit document |
| `bias-sentinel-no-should-be-fine` | No 'should be fine' / 'no impact': counter with a LINDDUN / OWASP cite | `agents/security-engineer.md` | ห้าม "should be fine" / "no impact" — counter ด้วย LINDDUN / OWASP cite |
| `bias-sentinel-false-positive-evidence` | A 'false positive' claim is refused without pasted evidence | `agents/security-engineer.md` | "เป็น false positive แน่ ๆ" — refuse without paste of evidence |
| `bias-stan-divergence-needs-doc` | Letting teams diverge is refused without an explicit 'accept divergence' document | `agents/staff-engineer.md` | — refuse without explicit "accept divergence" doc |
| `bias-tara-no-blind-vendor` | No blind acceptance of Bloomberg + FIX 4.4 when latency/venue/broker context does not need it | `agents/trading-expert.md` | ห้าม blindly accept Bloomberg + FIX 4.4 ถ้า latency tolerance > 50ms / single-venue / retail broker |
| `bias-tara-local-alternatives` | Consider local exchange native API + cheaper data vendors | `agents/trading-expert.md` | พิจารณา local exchange native API (SET ITCH/OUCH, KSE) + cheaper data vendor (Refinitiv, IEX, local) |
| `bias-tara-cite-before-vendor` | Cite cost + latency requirement + venue coverage before proposing a vendor | `agents/trading-expert.md` | ก่อน propose vendor → cite cost ($/user/year) + latency requirement + venue coverage |
...
| `fast-path-five-conditions` | Fast path is allowed only when ALL five conditions hold; one unknown = full workflow | `skills/workflow/ask/SKILL.md` | single-file deterministic fix · clear behaviour · no architecture change · no domain decision · no security boundary. One unknown → full workflow. |
| `fast-path-still-enforces` | Fast path still enforces evidence + R0/R1/R2 + Stop-and-return + the Phase 1c trigger; never drops a triggered role or gate | `skills/workflow/ask/SKILL.md` | Fast path still enforces evidence + R0/R1/R2 + Stop-and-return + the Phase 1c trigger; it never drops a triggered role or a gate. |
| `full-workflow-nine-triggers` | Full multi-agent workflow is mandatory when any of the nine triggers applies (security-sensitive = a Phase 1c trigger) | `skills/workflow/ask/SKILL.md` | new product behaviour · architecture decision · cross-domain impact · large feature · security-sensitive change (= a Phase 1c trigger) · significant UI flow · complex migration · multi-service contract · production deployment |
| `trust-cascade-no-upgrade` | Trust cascade: a chain never upgrades trust (level = min of the chain); the label is internal, the behaviour is not | `skills/discipline/shode-house-routing/SKILL.md` | trust level = min(A's level, A's output level). ห้าม upgrade trust ของ chain |
| `oliver-dispatch-floor` | Always-on floor (trigger -> action): Oliver never edits code/config nor runs the verification himself; FIRST, above the branches below, not Phase 1c/R0/incident (discipline § Ask vs derive): expected behaviour unclear or no evidenced cause after inspect/diagnose -> ask user (option-style), STOP, no Dave / edit / guessed fix; then inspect, then dispatch before any edit or verdict (bug/failing test: load diagnose, then Dave (evidenced cause only); live customer impact: incident instead; review Chris; verify/integration Quinn BEFORE Oliver runs anything - inspect = Read/Grep, never run the code under check; UI file Dave + ui-test/Uma, never Dave alone; other single-file deterministic change Dave alone + targeted test; everything else Dave + Chris); info-only question (no review/verify/ship verdict) needs no dispatch | `output-styles/oliver.md` | Oliver never edits code/config nor runs the verification himself — inspect, then dispatch BEFORE any edit or verdict: bug/failing test → load `diagnose` first |
| `oliver-phase1c-no-waiver` | Always-on Phase 1c no-waiver: a trigger means dispatch Sentinel now, before Phase 2, without asking permission to start; 'low risk'/user pressure never waives it and no skip / implement-in-parallel option is offered | `output-styles/oliver.md` | dispatch Sentinel now ก่อน Phase 2 — do not ask permission to start; "low risk"/user pressure never waives it — never offer a skip or implement-in-parallel option |
