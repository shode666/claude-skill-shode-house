...
| `oliver-dispatch-floor` | Always-on floor: Oliver never edits production code or issues a review/test verdict himself; every code edit is dispatched (fast path: Dave alone + targeted test; Bounded tier up: Dave + Chris; integration verdict Quinn; UI Uma/ui-test); load ask before the first edit | `output-styles/oliver.md` | Oliver never edits production code or issues a review/test verdict himself — every code edit is dispatched: fast path → Dave alone + targeted test |
| `oliver-phase1c-no-waiver` | Always-on Phase 1c no-waiver: a trigger means Sentinel before Phase 2; 'low risk'/user pressure never waives it and no skip / implement-in-parallel option is offered | `output-styles/oliver.md` | "low risk"/user pressure never waives it — never offer a skip or implement-in-parallel option |
