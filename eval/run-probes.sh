...
#   PROBE_IDS="P01 P02" bash eval/run-probes.sh      subset · PROBE_IDS=all = every kind:"probe" id
...
if [ "$IDS" = all ]; then
  IDS="$(python3 -c 'import json,sys;print(" ".join(s["id"] for s in json.load(open(sys.argv[1],encoding="utf-8"))["scenarios"] if s.get("kind")=="probe"))' "$SCENARIOS")"
fi
preflight
mkdir -p "$OUT" || die "cannot create $OUT"
OUT="$(cd "$OUT" && pwd -P)"
printf 'id\texit\tfirst_skill\tfirst_agent\tseconds\n' > "$OUT/SUMMARY.tsv"
BAD=0
for ID in $IDS; do
  FIRST_SKILL="-" FIRST_AGENT="-" SECONDS_TAKEN=0
  run_one "$ID" "$MODEL" "$OUT/$ID"; RC=$?
  [ "$RC" -ge 2 ] && BAD=$((BAD + 1))
  printf '%s\t%s\t%s\t%s\t%s\n' "$ID" "$RC" "$FIRST_SKILL" "$FIRST_AGENT" "$SECONDS_TAKEN" >> "$OUT/SUMMARY.tsv"
done
