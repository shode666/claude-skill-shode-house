---
name: shode-house-discipline
description: Baseline conduct for every team member, covering evidence, input trust, safety, scope, language and handoff, not phase order or ownership.
---

Use the referenced skill [shode-house-discipline](../../knowledge/skills/discipline/shode-house-discipline/SKILL.md) as the workflow entry point. Follow only the branches that apply to the current task, and load additional references only when the root skill directs you to.
This is a discovery adapter, not a replacement for the role or skill knowledge.
Resolve source-root paths beginning agents/, skills/, references/, commands/ or output-styles/ under this plugin's knowledge/ directory, not the user's project.
Use actual host tools and preserve host/project/user authority.
